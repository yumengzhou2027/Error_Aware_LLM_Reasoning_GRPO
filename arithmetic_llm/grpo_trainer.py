...
from .reward_functions import BaseRewardFunction, ScheduledReward
...
class GRPOTrainer:
    ...
    def __init__(
        ...
        self.reward_fn: Optional[BaseRewardFunction] = reward_fn
    ...
    def train_step(
        ...
        # Per-component reward accumulators for logging
        _fmt_scores: List[float] = []
        _proc_scores: List[float] = []
        _cons_scores: List[float] = []
        _out_scores: List[float] = []
            ...
            for cand_idx in range(num_candidates):
                ...
                if self.reward_fn is not None:
                    reward = self.reward_fn.compute_reward(
                        text, ground_truth[prompt_idx], prompt
                    )
                    rv = self.reward_fn.last_reward_vector
                    if rv is not None:
                        _fmt_scores.append(rv.format_score)
                        _proc_scores.append(rv.process_score)
                        _cons_scores.append(rv.consistency_score)
                        _out_scores.append(rv.outcome_score)
                else:
        ...
        # Advance adaptive scheduler if applicable
        if isinstance(self.reward_fn, ScheduledReward):
            avg_fmt = sum(_fmt_scores) / len(_fmt_scores) if _fmt_scores else 0.0
            avg_proc = sum(_proc_scores) / len(_proc_scores) if _proc_scores else 0.0
            self.reward_fn.step(format_score=avg_fmt, process_score=avg_proc)
        ...
        # Append per-component metrics when using pluggable reward
        if _fmt_scores:
            result["avg_format_score"] = sum(_fmt_scores) / len(_fmt_scores)
            result["avg_process_score"] = sum(_proc_scores) / len(_proc_scores)
            result["avg_consistency_score"] = sum(_cons_scores) / len(_cons_scores)
            result["avg_outcome_score"] = sum(_out_scores) / len(_out_scores)

        if isinstance(self.reward_fn, ScheduledReward):
            result["reward_phase"] = self.reward_fn.current_phase()
            w = self.reward_fn.current_weights()
            result["reward_weights"] = list(w)
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                if accum_metrics is None:
                    accum_metrics = {
                        key: (list(value) if isinstance(value, list) else 0.0)
                        for key, value in metrics.items()
                    }
                for key, value in metrics.items():
                    if isinstance(value, list):
                        accum_metrics[key] = list(value)
                    else:
                ...
                if did_step and global_step % self.config.log_every == 0:
                    ...
                    component_fragment = ""
                    if "avg_format_score" in avg_metrics:
                        component_fragment = (
                            f"fmt {avg_metrics['avg_format_score']:.3f} "
                            f"proc {avg_metrics['avg_process_score']:.3f} "
                            f"cons {avg_metrics['avg_consistency_score']:.3f} "
                            f"out {avg_metrics['avg_outcome_score']:.3f} "
                        )
                    phase_fragment = ""
                    if "reward_phase" in avg_metrics:
                        phase_fragment = f"phase {int(avg_metrics['reward_phase'])} "
                ...
                # Include per-component reward scores when available
                for key in (
                    "avg_format_score", "avg_process_score",
                    "avg_consistency_score", "avg_outcome_score",
                    "reward_phase", "reward_weights",
                ):
                    if key in log_metrics:
                        log_entry_metrics[key] = log_metrics[key]

                training_log.append({
                    "step": global_step,
                    "epoch": epoch + 1,
                    "batch_idx": batch_idx,
                    "metrics": log_entry_metrics,
