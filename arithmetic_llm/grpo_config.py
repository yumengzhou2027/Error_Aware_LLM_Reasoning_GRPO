...
class GRPOConfig:
    ...
    Attributes:
        ...
        reward_mode: Reward function to use ('outcome_only', 'naive_process',
            'error_aware', 'scheduled')
        reward_weights: (w_format, w_process, w_consistency, w_outcome)
        schedule_strategy: Scheduler strategy ('linear', 'cosine', 'threshold', 'fixed')
        schedule_phase1_frac: Fraction of training in Phase 1
        schedule_phase2_frac: Fraction of training in Phase 2
    """
    ...
    reward_weights: Tuple[float, float, float, float] = (0.10, 0.40, 0.20, 0.30)

    # --- Adaptive scheduling settings ---
    schedule_strategy: str = "linear"
    schedule_phase1_frac: float = 0.25
    schedule_phase2_frac: float = 0.50
    ...
    def validate(self) -> None:
        ...
        valid_reward_modes = (
            "outcome_only", "naive_process", "error_aware", "scheduled",
        )
        if self.reward_mode not in valid_reward_modes:
            raise ValueError(
                f"reward_mode must be one of {valid_reward_modes}, "
                f"got {self.reward_mode!r}"
            )

        if len(self.reward_weights) != 4:
            raise ValueError(
                f"reward_weights must have 4 elements, got {len(self.reward_weights)}"
            )
        if any(w < 0 for w in self.reward_weights):
            raise ValueError(
                f"reward_weights must all be non-negative, got {self.reward_weights}"
            )

        valid_strategies = ("linear", "cosine", "threshold", "fixed")
        if self.schedule_strategy not in valid_strategies:
            raise ValueError(
                f"schedule_strategy must be one of {valid_strategies}, "
                f"got {self.schedule_strategy!r}"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "schedule_phase1_frac": self.schedule_phase1_frac,
            "schedule_phase2_frac": self.schedule_phase2_frac,
        }
